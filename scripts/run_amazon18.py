...
def run_epoch(
    ...
    pbar = tqdm(loader)
    pbar.set_description("Training" if is_train else "Validating")
    for non_seq_x, seq_x, labels in pbar:
        ...
        pbar.set_postfix_str(f"Loss: {loss.item():.4f}, Acc: {accuracy_from_logits(logits, labels):.4f}")
