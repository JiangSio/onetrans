...
def convert_inters2dict_amazon18_style(reviews):
    ...
    return user2items, user2index, item2index, interactions #用户id:[物品id]，用户id映射，物品id映射，交互列表（用户id，物品id，评分，时间戳）
...
def generate_interaction_list_json2csv_style(reviews, user2index, item2index, id_title):
    ...
    # 处理用户信息：把用户的评论id，评分，时间戳，物品id，物品标题都保存在一个字典里，后续会根据时间戳排序生成交互序列
...
if __name__ == '__main__':
    ...
    # 过滤后评论list，用户统计，物品统计，元数据，原id到标题的映射
    ...
    #用户id:[物品id]，用户id映射，物品id映射，交互列表（用户原id，物品原id，评分，时间戳）
    ...
    # 生成交互序列 user原id ,item_asins (history), item_asin (target), history_item_id, item_id (target), history_item_title, item_title (target), history_rating, rating (target), history_timestamp, timestamp (target)
    ...
    # 划分保存文件，user_id:token\titem_id_list:token_seq\titem_id:token
